-- 학교
ALTER TABLE `USCLS`
	DROP FOREIGN KEY `FK_USERS_TO_USCLS`; -- 회원 -> 학교

-- 경력
ALTER TABLE `UEXPS`
	DROP FOREIGN KEY `FK_USERS_TO_UEXPS`; -- 회원 -> 경력

-- 기업정보
ALTER TABLE `CINFOS`
	DROP FOREIGN KEY `FK_USERS_TO_CINFOS`; -- 회원 -> 기업정보

-- 구인공고
ALTER TABLE `CRECS`
	DROP FOREIGN KEY `FK_CINFOS_TO_CRECS`; -- 기업정보 -> 구인공고

-- 기업사진
ALTER TABLE `CPHOTS`
	DROP FOREIGN KEY `FK_CINFOS_TO_CPHOTS`; -- 기업정보 -> 기업사진

-- 기업태그
ALTER TABLE `CTAGS`
	DROP FOREIGN KEY `FK_CINFOS_TO_CTAGS`; -- 기업정보 -> 기업태그

-- 자격증
ALTER TABLE `UCERTS`
	DROP FOREIGN KEY `FK_USERS_TO_UCERTS`; -- 회원 -> 자격증

-- 외국어
ALTER TABLE `ULANS`
	DROP FOREIGN KEY `FK_USERS_TO_ULANS`; -- 회원 -> 외국어

-- 기술연차
ALTER TABLE `YSKILLS`
	DROP FOREIGN KEY `FK_SKILLS_TO_YSKILLS`; -- 보유기술 -> 기술연차

-- 기술연차
ALTER TABLE `YSKILLS`
	DROP FOREIGN KEY `FK_USERS_TO_YSKILLS`; -- 회원 -> 기술연차

-- 기업등록프로젝트
ALTER TABLE `CPJTS`
	DROP FOREIGN KEY `FK_CINFOS_TO_CPJTS`; -- 기업정보 -> 기업등록프로젝트

-- 프로젝트경력
ALTER TABLE `UPJTNO`
	DROP FOREIGN KEY `FK_USERS_TO_UPJTNO`; -- 회원 -> 프로젝트경력

-- 프로젝트정보문의게시판
ALTER TABLE `PJTBDS`
	DROP FOREIGN KEY `FK_CPJTS_TO_PJTBDS`; -- 기업등록프로젝트 -> 프로젝트정보문의게시판

-- 프로젝트정보문의게시판
ALTER TABLE `PJTBDS`
	DROP FOREIGN KEY `FK_USERS_TO_PJTBDS`; -- 회원 -> 프로젝트정보문의게시판

-- 프로젝트첨부파일
ALTER TABLE `CPJTATCHS`
	DROP FOREIGN KEY `FK_CPJTS_TO_CPJTATCHS`; -- 기업등록프로젝트 -> 프로젝트첨부파일

-- 회원사진
ALTER TABLE `UPHOTS`
	DROP FOREIGN KEY `FK_USERS_TO_UPHOTS`; -- 회원 -> 회원사진

-- 회원태그
ALTER TABLE `UTAGS`
	DROP FOREIGN KEY `FK_USERS_TO_UTAGS`; -- 회원 -> 회원태그

-- 구인정보첨부파일
ALTER TABLE `CREATCHS`
	DROP FOREIGN KEY `FK_CRECS_TO_CREATCHS`; -- 구인공고 -> 구인정보첨부파일

-- 구인정보필요기술
ALTER TABLE `CRESKILLS`
	DROP FOREIGN KEY `FK_CRECS_TO_CRESKILLS`; -- 구인공고 -> 구인정보필요기술

-- 구인정보필요기술
ALTER TABLE `CRESKILLS`
	DROP FOREIGN KEY `FK_SKILLS_TO_CRESKILLS`; -- 보유기술 -> 구인정보필요기술

-- 프로젝트필요기술
ALTER TABLE `CPJTSKILLS`
	DROP FOREIGN KEY `FK_CPJTS_TO_CPJTSKILLS`; -- 기업등록프로젝트 -> 프로젝트필요기술

-- 프로젝트필요기술
ALTER TABLE `CPJTSKILLS`
	DROP FOREIGN KEY `FK_SKILLS_TO_CPJTSKILLS`; -- 보유기술 -> 프로젝트필요기술

-- 구인정보문의게시판
ALTER TABLE `REBDS`
	DROP FOREIGN KEY `FK_CRECS_TO_REBDS`; -- 구인공고 -> 구인정보문의게시판

-- 구인정보문의게시판
ALTER TABLE `REBDS`
	DROP FOREIGN KEY `FK_USERS_TO_REBDS`; -- 회원 -> 구인정보문의게시판
...
CREATE TABLE `USCLS` (
	...
	`SCGR` INTEGER     NOT NULL COMMENT '학교성적', -- 학교성적
...
CREATE TABLE `CINFOS` (
	...
	`UNO`   INTEGER      NULL     COMMENT '회원일련번호' -- 회원일련번호
)
COMMENT '기업정보';
...
CREATE TABLE `CRECS` (
	...
	`REED`  DATE         NOT NULL COMMENT '마감일', -- 마감일
...
CREATE TABLE `CPHOTS` (
	...
	`CPH`   VARCHAR(255) NOT NULL COMMENT '사진', -- 사진
	`CNO`   INTEGER      NOT NULL COMMENT '기업일련번호' -- 기업일련번호
)
COMMENT '기업사진';
...
CREATE TABLE `CPJTS` (
	...
	`CPJYR`  INTEGER      NULL     COMMENT '프로젝트경력연차', -- 프로젝트경력연차
	...
	`CPJED`  DATE         NOT NULL COMMENT '마감일', -- 마감일
	`CPJSD`  DATE         NOT NULL COMMENT '시작일', -- 시작일
...
CREATE TABLE `CPJTATCHS` (
	...
	`CPJFNM` VARCHAR(255) NOT NULL COMMENT '첨부파일', -- 첨부파일
	`CPJNO`  INTEGER      NOT NULL COMMENT '등록프로젝트일련번호' -- 등록프로젝트일련번호
)
COMMENT '프로젝트첨부파일';
...
CREATE TABLE `CREATCHS` (
	...
	`REFNM` VARCHAR(255) NOT NULL COMMENT '첨부파일', -- 첨부파일
	`RENO`  INTEGER      NOT NULL COMMENT '구인일련번호' -- 구인일련번호
)
COMMENT '구인정보첨부파일';
...
-- 기업정보
ALTER TABLE `CINFOS`
	ADD CONSTRAINT `FK_USERS_TO_CINFOS` -- 회원 -> 기업정보
		FOREIGN KEY (
			`UNO` -- 회원일련번호
		)
		REFERENCES `USERS` ( -- 회원
...
insert into CPJTS VALUES(0,'title','description','1','5',10,2000,3000,10,20160424,20160425,20160426,'user','home','경기도 성남시',1);
insert into CPJTS VALUES(0,'title','description','1','5',10,3000,4000,5,20160424,20160426,20160427,'user','home','서울시 강남구',1);
insert into CPJTS VALUES(0,'title','description','1','5',10,4000,5000,15,20160424,20160427,20160428,'user','home','서울시 구로구',1);
...
insert into CRESKILLS VALUES(1,6);
insert into CRESKILLS VALUES(1,8);
insert into CRESKILLS VALUES(1,10);
insert into CRESKILLS VALUES(1,34);
insert into CRESKILLS VALUES(1,29);
insert into CRESKILLS VALUES(1,14);
insert into CRESKILLS VALUES(1,23);
insert into CRESKILLS VALUES(1,27);
insert into CPJTSKILLS VALUES(1,1);
insert into CPJTSKILLS VALUES(1,4);
insert into CPJTSKILLS VALUES(1,7);
insert into CPJTSKILLS VALUES(1,10);
insert into CPJTSKILLS VALUES(1,14);
insert into CPJTSKILLS VALUES(1,40);
